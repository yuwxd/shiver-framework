...
class CommandRegistry {
    ...
    async syncToDiscord(client, options = {}) {
        ...
        for (const cmd of commands) {
            if (!cmd.data || cmd.disabled) continue;
            try {
        ...
        try {
            ...
            } else if (typeof client.application.fetch === 'function') {
                await client.application.fetch().catch(() => {});
            }
        } catch (_) {}
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                console.log(
                    `[CommandRegistry] Slash sync OK: ${payloads.length} command(s)` +
                        (appCommands ? ' (global)' : '') +
                        (guildTotal ? `, guilds ${guildSynced}/${guildTotal}` : '')
                );
                break;
            } catch (err) {
    ...
    getByCategory(category) {
        return this.getAllSlash().filter(cmd => cmd.category === category);
    }
